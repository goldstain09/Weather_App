...
export default function InputAndInfo({ gettingDAYNIGHT_Value }) {
  ...
  const Checking = async (e) => {
    ...
    let Input_value = e.target.value.charAt(0).toUpperCase() + e.target.value.slice(1);
    ...
    try {
      ...
      if (Input_value.length > 1) {
        let f_response = await fetch(`https://api.weatherapi.com/v1/current.json?key=76d3d506f3054689924160021231206&q=${Input_value}&aqi=no`);
      ...
      <div className='main_Container'>
        <div className='text-left Info_Div'>
          <input
            ...
            autoComplete='on' />
          {
            received && (<div className='d-flex gap-5 justify-content-between'>
              <div><h1 className='place_name'>{data.location.name}</h1></div>
              <div><h1 className='region_country'>{`${data.location.region}, ${data.location.country}`}</h1></div>
              <div><h1 className='live_time'>{liveTime.toLocaleString()}</h1></div>
            </div>)
          }
          {
            locationError && (<div className='d-flex'>
        ...
        {/* ------------------OTHERS------------------- */}
        {
          received && (<>   <div className='other_Info_Div d-flex row'>
            <div className='col col-12 col-sm-12 col-md-4 col-lg-4 col-xl-4 '>
              <div className='d-flex gap-5'>
                <img src={data.current.condition.icon} className='weather_condition_img' />
        ...
        {
          locationError && (<>   <div className='other_Info_Div d-flex row'>
            <MagnifyingGlass
              visible={true}
              height="300"
              width="300"
